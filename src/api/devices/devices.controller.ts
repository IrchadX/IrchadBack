...
export class DevicesController {
  ...
  @Patch(':id/set-user')
  async setUser(@Param('id') id: string, @Body('user_id') userId: number) {
    const updatedDevice = await this.service.setUser(+id, userId);
  ...
  async getDevicesNotAssigned() {
    try {
      ...
      throw new HttpException(
        'Failed to fetch unassigned devices',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getDevicesAssigned() {
    try {
      ...
      throw new HttpException(
        'Failed to fetch assigned devices',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getDeviceById(@Param('id') id: string) {
    try {
      ...
      throw new HttpException(
        'Failed to fetch device77777777777',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getStateTypeById(@Param('id') id: string) {
    try {
      ...
      throw new HttpException(
        'Failed to fetch state type by state ID',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
